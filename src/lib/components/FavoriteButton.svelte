...
{#if userLists?.favorites.map((item) => item.id).includes(book.id)}
	<form action="?/removeFromFavorites" method="POST" use:enhance={handleLoading}>
...
{:else}
	<form action="?/addToFavorites" method="POST" use:enhance={handleLoading}>
		<input type="hidden" value={book.id} id="id" name="id" />
